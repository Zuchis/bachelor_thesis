...
[DESCREVER MELHOR O SISTEMA DE TRAÇOS]

O sistema de traços é utilizado para escrever 
...
Todos os vértices presentes em uma cena, desde os seus valores crus até passarem pelo 
...
combinação intermediária (e g. Apenas translação e escala). 
...
Shaders são pequenos programas que são escritos em sua própria linguagem, chamada de 
linguagem de \textit{shading}, projetada para tornar a programação dos shaders fácil. 
...
como: transformar os vértices dos objetos de um espaço para outro (i.e espaço do objeto 
para espaço do mundo), sombreamento, utilizar as coordenadas de normais para 
iluminação, \textit{mappings}, utilizar coordenadas de textura para aplicar uma certa 
...
\begin{itemize}
    ...
    \item Fragment Shader: responsável pelo processamento dos fragmentos, isto são 
    ...
    \item Compute Shader: neste shader será feita a computação de informação arbitrária, 
...
O renderizador gráfico de baixo nível é responsável pelos elementos mais crus da 
...
comunicação é feita através de uma API gráfica, como por exemplo, o 
OpenGL~\cite{shreiner2013opengl}, que apesar de possuir uma vasta quantidade de comandos 
diferentes para uma rica manipulação dos recursos da GPU, geralmente necessidade de uma 
grande quantidade de código para fazer até mesmo as rotinas mais simples de uma aplicação 
gráfica. Por isso, além de utilizar uma API gráfica, esse componente responsável pela 
renderização de baixo nível também geralmente inclui um \textit{wrapper} para os comandos 
da API, condensando vários funções diferentes em abstrações de maior nível, agilizando 
o desenvolvimento e minimizando a quantidade de erros.
...
\subsubsection{Transformação de Modelo e de Visualização}
...
\subsubsection{\textit{Vertex Shading}}
...
\subsubsection{Mapeamento para Tela}
...
\subsubsection{Configuração dos Triângulos}

Nesta etapa os dados sobre as superfícies dos triângulos são computados. Esses dados são 
utilizados para a interpolação de várias informações de \textit{shading} produzida na 
etapa de geometria. Etapa feita por operações fixas do hardware dedicado para essa parte.

\subsubsection{Triangle Traversal}

Nessa etapa são procurados os pixeis que estão dentro de um triângulo, ou seja, que 
pertencem ao plano formado pelos três vértices de um dos triângulos presentes na tela. 
Esse processo de procura é conhecido como \textit{triangle traversal}. Para cada um dos 
pixeis encontrados é gerado um fragmento. As propriedades dos fragmentos de cada 
triângulo são geradas utilizando dados interpolados entre os três vértices.

\subsubsection{Pixel Shading}

Nessa etapa são feitas todas as computações de \textit{shading} pixel por pixel, usando 
os dados interpolados como input, resultando em uma ou mais cores para serem passadas à 
próxima etapa. Esta etapa, ao contrário das outras que são feitas por funções fixas do 
hardware dedicado, é feita por núcleos programáveis da GPU, nomeadamente pelo 
\textit{fragment shader}. Esse shader será responsável por aplicar texturas, iluminação, 
entre outros em cada um dos fragmentos.

\subsubsection{Combinação}

Por fim, a informação de cada pixel é armazenada em um \textit{buffer} de cores, que 
contém cada um dos componentes RGB da cor final. O a unidade da GPU responsável por essa 
etapa não é completamente programável, porém altamente customizável.
...
Além de utilizar os conceitos da modelagem orientada a dados, também foi necessário 
...
capítulo~\ref{cap2} que fazem parte da estrutura de um motor de jogos, como o 
...
pois seu objetivo é dar suporte ao desenvolvimento de uma outra aplicação, uma aplicação 
gráfica. Ou seja, um motor de jogos é um sistema, que não fornece serviços diretamente 
para o usuário, mas sim serviços para se produzir outro software. Por esse motivo, além 
do desenvolvimento do motor de jogos, será necessário o desenvolvimento de uma aplicação 
que utilize as funcionalidades do motor para realmente verificar sua eficiência.
...
dados. Isso permitirá duas coisas: a primeira será a comparação entre essas duas 
aplicações e a análise de eficiência da MOD, a segunda será a demonstração de como uma 
aplicação com orientação a objetos pode ser convertida para um equivalente com orientação 
a dados.
...
considerável parcela dos componentes necessitam a sua utilização. Além disso, todas as 
...
portanto seu desenvolvimento seria redundante.
