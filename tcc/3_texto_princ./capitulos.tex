...
O método \textit{draw} é o principal do grafo de cena, por ser utilizado no loop principal 
da aplicação para controlar a renderização do grafo. Na figura

\begin{lstlisting}[frame=single]            
void draw() {
    if (object == nullptr) {
        throw RenderException("Nó sem objeto");
    }
    if (shaderProgram == nullptr) {
        shaderProgram = getProgramFromParent();
        if (shaderProgram == nullptr) {
            throw RenderException("Nó sem shader");
        }
    }
    shaderProgram->use();
    shaderProgram->setUniform("Matrix",this->getModelMatrix());
    object->draw();
    if (!children.empty()){
        int i;
