...
\section{Projeto Orientado a Dados}
\label{secdataorienteddesign}

O projeto orientado a dados (do inglês: \textit{Data Oriented Design}) é 
...
Um exemplo é o trabalho de Chellappa~\cite{chellappa2008write}, no 
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \par\medskip
    Adaptado de~\cite{gamesfromwithin}.
    \label{oodvsdod}
\end{figure}
...
preceitos de boa engenharia de software segundo Kruchten et al.~(\citeyear{Kruchten2006}).
...
muito além do escopo deste trabalho, Verth e Bishop~(\citeyear{Verth:2008}) e 
Mortenson~(\citeyear{mortenson1999mathematics}) fazem uma 
...
populares pois têm um baixo custo computacional em termos de processamento de 
...
truncada, chamada de \textit{frustum} de visualização. 

A figura~\ref{viewingfrustum} ilustra um exemplo de um \textit{frustum} de visualização, 
na qual o objeto representado por uma esfera está visível para espectador, pois 
está entre os planos que delimitam o \textit{frustum}. O plano menor é conhecido é 
conhecido como plano "perto", e o plano maior (base da pirâmide) é conhecido como plano 
"longe". Quanto mais próximo um objeto se encontra da base da pirâmide, menor ele 
aparenta ser~\cite{hughes2014computer}.

\begin{figure}[h!]
    \centering
    \captionof{figure}{\textit{Frustum} de visualização no qual as extremidades da 
    pirâmide truncada estão representadas pelos planos.}
    \includegraphics[width =\textwidth]{../figuras/viewingfrustum}
    \par\medskip
    Fonte:~\cite{pinhocg}
    \label{viewingfrustum}
\end{figure}
...
do ponto de vista da câmera. Para a maior parte dos modelos de hardware gráfico disponíveis, 
isso é feito com 
...
paramétricas, a seguir serão apresentados alguns exemplos de interpolações que foram
utilizadas no trabalho desenvolvido. Essas interpolações e também outras não mencionadas 
estão disponíveis para consulta no trabalho de Verth e Bishop~(\citeyear{Verth:2008}). 

Dado os dois pontos sendo interpolados, uma interpolação entre estes 
...
A projeto orientado a dados é um termo que surgiu recentemente, porém seu conceito de 
...
Também foram discutidos alguns de seus problemas, como a necessidade de diferentes 
...
jogos, frequentemente levando a problemas de comunicação e pesquisa deficiente e confusão 
...
o desenvolvimento de um motor de jogos. Em seu trabalho, eles mencionam os problemas 
...
Um trabalho mais relacionado ao tema projeto orientado a dados é o proposto 
...
\chapter{Otimizações para motores de jogos através de projeto 
orientado a dados}
\label{proposalcap}

Para testar as otimizações proporcionadas pelo projeto orientado 
...
objetos, e outra utilizando projeto orientado a dados,
...
outra versão com a abordagem de projeto orientado a dados será referida 
...
de acesso à memória para a versão orientada a objetos e um mal padrão de acesso para a 
...
ultrapassa os limites da cena, arbitrariamente escolhidos.
...
Com o fluxo de dados definido, assim como as transformações 
necessárias sobre os dados, é possível determinar quais são os dados 
...
\begin{figure}[h]
    \centering
    \captionof{figure}{Estruturas utilizadas na versão orientada a dados do motor.}
...
desenvolvida uma biblioteca matemática o mais minimalista o possível, contendo somente 
...
e a segunda para verificar se o nó está sem shader. Na linha 11 
...
de dados no CPU, apenas chamadas de funções do \textit{openGL} para renderizar 
os objetos dos nós do grafo, cujo processamento é feito no GPU. Esta 
última etapa utiliza três propriedades dos nós: a matriz de 
coordenadas globais, os dados do shader e dados da malha.
...
Algumas das funcionalidades comumente presentes em um gerenciador 
de recursos incluem~\cite{gregory2009game}:
\begin{itemize}
...
\textit{Managers} conforme discutido no capítulo~\ref{proposalcap}. Os \textit{Managers}
...
Na versão OD do motor, os métodos das classes foram separados 
...
abordagem com projeto orientado a dados conforme discutido 
...
projeto orientado a dados como uma forma de otimização de 
...
relacionadas com nenhuma das estruturas também foram listadas na 
seção~\ref{mathutilitiessec}.
...
limite de 3200 objetos em cena, foi realizado um pré-teste das 
aplicações, incrementando-se a quantidade de objetos em cada 
execução e medindo a taxa média de FPS em cada uma dessas execuções.
Ao executar as aplicações com 3200 objetos em cena, obteve-se uma 
taxa média de 60 FPS, que é considerado a taxa de quadros por segundo mínima no 
atual mercado de jogos para se ter uma experiência agradável ao 
jogador.
...
A seguir serão discutidos os dois cenários desenvolvidos para 
a realização dos testes de desempenho dos motores, denominados 
como problema A e problema B. Os cenários são quase idênticos, 
tendo como única diferença a hierarquia entre os nós do 
grafo de cena. 

Na inicialização da aplicação, os recursos 
externos (\textit{shaders} e malhas) são carregados, depois 
os objetos são inicializados com valores aleatórios em 
suas propriedades (posição, velocidade, aceleração, rotação, etc.). 
O último passo da inicialização é a construção do grafo de cena e 
sua hierarquia, que consiste em configurar os \textit{shaders}, 
malhas e a referência para o pai de cada nó.

Depois da inicialização a aplicação entra no \textit{loop} 
principal, o qual é executado uma vez por quadro. O 
\textit{loop} consiste nos seguintes passos: atualização 
das propriedades do objeto, detecção e processamento de 
colisões e por fim cálculo das coordenadas locais e globais 
seguido da renderização do grafo de cena. 

O componente de colisão não detecta colisões entre objetos, 
somente entre os objetos e os limites da cena, cada uma das 
dimensões (X, Y e Z), possuem limites superiores e inferiores. 
Nessa etapa o processamento de dados é feito somente quando 
ocorrem colisões, por isso este método é consideravelmente 
menos custoso do que os demais.
...
do nó raiz. Neste cenário um grafo de cena normalmente 
...
\cline{2-7}
                                       & \multicolumn{2}{c|}{Draw}                                     & \multicolumn{2}{c|}{Att. objetos}                              & \multicolumn{2}{c}{Verif. colisões}                \\ \hline
\multicolumn{1}{l|}{Número de objetos} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & Média           & \multicolumn{1}{l}{Desv. padrão} \\ \hline
100                                     & 0.62                       & 0.05                              & \textbf{0.02}              & 0.005                             & $\bm{6 . 10^{-3}}$   & $7 . 10^{-3}$                      \\ \hline
200                                     & 1.04                       & 0.01                              & \textbf{0.04}              & 0.003                             & \textbf{0.001}           & $2 . 10^{-3}$                      \\ \hline
400                                     & 1.96                       & 0.05                              & \textbf{0.11}              & 0.01                              & \textbf{0.002}           & 0.001                              \\ \hline
800                                     & 3.84                       & 0.05                              & \textbf{0.22}              & 0.03                              & \textbf{0.004}           & 0.002                              \\ \hline
1600                                    & 7.65                       & 0.06                              & \textbf{0.45}              & 0.01                              & \textbf{0.008}           & $2 . 10^{-3}$                      \\ \hline
3200                                    & 15.28ms                    & 0.1                               & \textbf{0.91}              & 0.01                              & \textbf{0.016}           & $6 . 10^{-3}$                      \\ \hline
\end{tabular}
\end{table}
...
\cline{2-7}
                                       & \multicolumn{2}{c|}{Draw}                                     & \multicolumn{2}{c|}{Att. objetos}                              & \multicolumn{2}{c}{Verif. colisões}                \\ \hline
\multicolumn{1}{l|}{Número de objetos} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & Média           & \multicolumn{1}{l}{Desv. padrão} \\ \hline
100                                     & 1.02                       & 0.01                             & 0.03                      & 0.002                               & 0.004           & $7 . 10^{-3}$                     \\ \hline
...
\cline{2-7}
                                       & \multicolumn{2}{c|}{Draw}                                     & \multicolumn{2}{c|}{Att. objetos}                              & \multicolumn{2}{c}{Verif. colisões}                \\ \hline
\multicolumn{1}{l|}{Número de objetos} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & \multicolumn{1}{l}{Média} & \multicolumn{1}{l|}{Desv. padrão} & Média           & \multicolumn{1}{l}{Desv. padrão} \\ \hline
100                                     & \textbf{0.54}              & 0.02                              &\textbf{0.03}                      & 0.002                      &$\bm{6 . 10^{-3}}$      & $2 . 10^{-4}$                     \\ \hline
200                                     & \textbf{1.04}              & 0.01                              &\textbf{0.07}                      & 0.006                      &\textbf{0.001}          & $2 . 10^{-3}$                     \\ \hline
400                                     & \textbf{1.95}              & 0.01                              &\textbf{0.18}                      & 0.01                       &\textbf{0.004}          & 0.003                             \\ \hline
800                                     & \textbf{3.85}              & 0.04                              &\textbf{0.38}                      & 0.02                       &\textbf{0.05}           & 0.03                              \\ \hline
1600                                    & \textbf{7.62}              & 0.03                              &\textbf{0.69}                      & 0.01                       &\textbf{0.008}          & $2 . 10^{-3}$                     \\ \hline
3200                                    & \textbf{15.29}             & 0.06                              &\textbf{1.31}                      & 0.01                       &\textbf{0.01}           & $2 . 10^{-3}$                     \\ \hline
\end{tabular}
\end{table}
...
Neste capítulo foi testado o desempenho do projeto orientado
...
SoA no problema A obteve um desempenho em média mais lento do
que a versão OO no método \textit{draw}, que é o que utiliza a 
maior quantidade de propriedades das entidades. Teve um aumento 
insignificante de desempenho no método de atualização de objetos,
o segundo a utilizar mais propriedades, e por fim obteve um 
desempenho em média dez vezes mais rápido no método de verificação 
de colisões, que utiliza somente duas propriedades das entidades.
