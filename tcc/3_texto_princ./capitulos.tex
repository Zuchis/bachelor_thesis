...
Rust é uma linguagem de programação nova cuja primeira versão estável foi lançada em 2015.
...
tanto de linguagens imperativas quanto de linguagens funcionais. Exemplos de 
...
que permite a construção de abstrações.

O sistema de traços é uma característica da linguagem. Um traço pode ter uma ou mais 
funcionalidades, se for declarado que um certo tipo de variável possui esse traço, isso 
diz ao compilador do Rust que aquele tipo deve fornecer as funcionalidades descritas 
no traço~\cite{rustdocs}.

\begin{figure}[h]
    \centering
    \captionof{figure}{Demonstração do sistema de traços do Rust.}
    \includegraphics[width =.8\textwidth]{../figuras/rusttraitsystem}
    \par\medskip
    Fonte: \cite{rustdocs}.
    \label{traitsystemdemo}
\end{figure}

A figura~\ref{traitsystemdemo} demonstra um exemplo de uso do sistema de traços. Neste 
exemplo, há uma estrutura chamada \textit{Circle}, e um traço chamado \textit{HasArea}, 
esse traço possui a função \textit{area}. Através da palavra-chave \textit{impl}, o 
traço \textit{HasArea} é implementado para a estrutura do exemplo, outras estruturas 
poderiam ser criadas e para estas esse traço também poderia ser implementado.

Quando uma dessas funções de um traço é chamada para algum tipo de dado específico, o 
compilador gera uma variante daquela função a qual terá ligação estática total, 
permitindo a aplicação de otimizações~\cite{holk2013}.
...
O sistema de coordenadas do objeto é considerado o sistema de coordenadas "cru" deste. 
Neste sistema as coordenadas são locais e o ponto de referência é o centro do objeto. 
Quando se define uma malha de polígonos, os vértices estão nesse sistema de coordenadas.
...
vértices são posicionados conforme o ponto de vista de uma câmera.

Uma câmera é um objeto abstrato de uma cena o qual possui uma posição e uma orientação, 
e é nesse estágio que os objetos da cena são posicionados de acordo com esses dois 
parâmetros.
...
transforma o volume de visualização em uma pirâmide truncada. O topo da pirâmide é 
chamado de plano perto, e é o início da visualização da câmera. A base da pirâmide é 
chamada de plano longe, e é o fim da visualização da câmera. Quanto mais próximo os 
objetos estão da base da pirâmide, menor eles irão aparentar na cena.
...
Esse estágio não é dependente de nenhum argumento da aplicação e é feito automaticamente 
...
anterior, este estágio é feito automaticamente pelo hardware.
...
transformações afins e projeções de perspectiva, e a maior parte dos hardwares 
comerciais de aceleração gráfica são otimizados para rasterização de 
triângulos~\cite{gregory2009game}.

\section{Iluminação}

Iluminação em aplicações gráficas consiste na simulação do modelo de iluminação que temos 
do mundo real. Existem várias fontes de luzes diferentes e também diferentes tipos de 
...
diferente do \textit{pipeline} de renderização. A seguir será listada os diferentes 
...
\begin{itemize}
    ...
    \item Compute Shader: neste shader será feita o cálculo de informação arbitrária, 
        geralmente utilizado para tarefas que não estão diretamente relacionadas com a 
        renderização de triângulos ou pixeis.
\end{itemize}
...
renderização. Sua preocupação é lidar com as primitivas geométricas da maneira mais 
eficiente o possível sem comprometer suas integridades~\cite{gregory2009game}.
...
manipulação dos recursos da GPU, geralmente necessidade de uma grande quantidade de 
...
Alguns exemplos de computações que são feitas nesse estágio são: gerenciamento de inputs, 
detecção de colisão, animações de texturas, animações através de transformações, ajuste 
de parâmetros que são utilizados nas outras etapas, etc.

Ao final dessa fase, toda a geometria e parâmetros computados são passados à fase de 
geometria, esse é considerado o passo mais importante.
...
truncada, chamada de \textit{frustum}, na qual os objetos mais próximos da base da 
pirâmide aparentam ser menores.
...
$y$ de cada primitiva são transformadas para formar as coordenadas tela. As coordenadas 
...
pixeis são computadas e definidas, esse é o processo conhecido como rasterização, que é 
a conversão dos vértices em coordenadas de janela juntamente com a informação de 
\textit{shading} para pixeis desenhados na tela.
...
Na etapa de \textit{triangle traversal} são procurados os pixeis que estão dentro de 
um triângulo, ou seja, que pertencem ao plano formado pelos três vértices de um dos 
triângulos presentes na tela. Esse processo de procura é conhecido como 
\textit{triangle traversal}. Para cada um dos pixeis encontrados é gerado um 
...
por pixel, usando os dados interpolados como input, resultando em uma ou mais cores 
...
de cores, que contém cada um dos componentes RGB da cor final. O a unidade da GPU 
...
O loop do jogo é onde os componentes funcionam em conjunto para proporcionar o que 
define um jogo: uma aplicação interativa em tempo real. 

Sistemas em tempo real, como jogos, possuem restrições de tempo para realizar todas as 
suas rotinas, se o sistema não for capaz de fazer seu trabalho dentro do limite de 
tempo irá falhar. Uma vez que sistemas em tempo real são supostos a fazer suas tarefas 
...
interatividade que deveria. Um loop de jogo pode ser implementado com a finalidade 
de satisfazer essas restrições~\cite{Joselli2010}.

Sistemas interativos em tempo real podem ser divididos em três módulos principais: 
recebimento de inputs, processamento e apresentação dos 
resultados~\cite{dalmau2004core}.

Nos jogos eletrônicos, o sistema de inputs corresponde ao gerenciamento do dispositivo 
de inputs, como mouse, teclado ou controlador de jogo, o processamento é responsável 
...
Elaborar um loop de jogo significa organizar esses três módulos, e seus submódulos na 
certa para deixar a simulação o mais fiel o possível da experiência desejada. 
Considerando os módulos citados, o loop do jogo pode ser dividido da seguinte maneira, 
na seguinte ordem:

\begin{itemize}
    \item Detecção e gerenciamento de inputs
...
frequência com a qual esses loops são executados é a quantidade de quadros por 
...
quantidade de quadros construídos, uma mesma sequência de eventos podem ter um 
...
não precisa ser atualizada com a mesma frequência do que a simulação das físicas, 
...
relação à taxa de quadros por segundo com a qual a aplicação está rodando, e como esse 
problema podia ser solucionado adicionando um parâmetro que indicasse o tempo decorrido 
entre o último quadro e o atual.
...
alguns exemplos de componentes comum em motores de jogos.
...
Rust é uma linguagem de programação nova e promissora por apresentar um desempenho 
semelhante às linguagens C e C++ porém apresenta um nível maior de de segurança de 
memória. Foi feita uma breve introdução da linguagem, juntamente com seus objetivos 
e foram explicadas algumas características que garantem que a linguagem consiga 
cumpri-los.
...
dados desde o seu carregamento no sistema até serem transformados em pixeis na tela, 
...
Todos os componentes são combinados para a formação de um loop principal, que será 
encarregado de receber e gerenciar os \textit{inputs} do jogador, atualizar os 
componentes que necessitam fazê-lo, e apresentar os resultados computados. Esse loop 
é conhecido como loop de jogo, neste capítulo esse conceito foi introduzido e detalhado.
...
Neste capítulo serão apresentados alguns trabalhos na literatura que estão relacionados 
com a proposta deste trabalho. 

A maioria dos trabalhos apresentados concentra-se em criar funcionalidades únicas para 
um motor de jogos e também nas melhores práticas para o desenvolvimento de motores de 
jogos, porém nenhum destes trabalhos explora o uso da modelagem orientada a dados.
...
Freitas~\cite{deFreitas2012GEC} propôs um modelo de motor de jogos com uma 
...
comerciais populares, como é o caso do motor de jogos de propósito geral Unity3D. Nesse 
...
O trabalhos deles prossegue então, para explicar seus diferenciais, como a utilização de 
entidades dinâmicas, que permite a reconfiguração dinâmica das entidades em diferentes 
momentos da aplicação. Consequentemente isso permite flexibilidade e extensibilidade 
até mesmo em tempo de execução. As decisões de design que os permitiram atingir esses 
objetivos são então apresentadas.

Anderson~\cite{Anderson2008} levanta em seu trabalho alguns problemas ainda pendentes 
...
podem ser potenciais áreas de pesquisa a respeito de motores de jogos, tais problemas 
...
específico beneficiam somente uma classe específica de jogos. É levantada então a 
...
evolução da tecnologia utilizada em hardwares ou nas interfaces do software e suas 
capacidades, e como essas mudanças podem afetar a evolução de um ou múltiplos 
componentes do motor de jogos.
...
Keenan~\cite{Keenan2011} salienta a importância da definição de uma boa arquitetura para 
...
Como o seu trabalho é para fins educativos, o restante de seu trabalho trata sobre a 
...
Zhu~\cite{Zhu2016ECG} propõe uma nova metodologia para o desenvolvimento de jogos, 
...
sobre maneiras de melhorar a arquitetura e projeto de um motor de jogos, ou proporcionar 
...
Neste capítulo será explicada a proposta deste trabalho, um motor de jogos escrito em Rust 
com modelagem orientada a dados visando à otimização da comunicação entre o processador e 
a memória.

Além de utilizar os conceitos da modelagem orientada a dados, também é necessário 
utilizar conceitos de computação gráfica, álgebra linear e geometria analítica, teoria de 
grafos, processamento de imagens, física, entre outros.
...
Como um motor de jogos por si só não é o suficiente para se gerar análises e resultados, 
pois seu objetivo é dar suporte ao desenvolvimento de uma outra aplicação nesse caso 
um jogo, podendo ser outra aplicação gráfica mais genérica. Ou seja, um motor de jogos 
é um sistema, que não fornece serviços diretamente para o usuário, mas sim serviços 
para se produzir outro software. Por esse motivo, além do desenvolvimento do motor de 
jogos, será necessário o desenvolvimento de uma aplicação que utilize as 
funcionalidades do motor para realmente verificar sua eficiência.

Para que seja possível um meio de comparação, será desenvolvida uma aplicação simples que 
utilize os módulos do motor de jogos em duas versões diferentes, uma delas será feita 
com programação orientada a objetos, e na outra será utilizada modelagem orientada a 
dados. Isso permitirá a comparação entre essas duas aplicações e a análise de 
eficiência da MOD, bem como a demonstração de como uma aplicação com orientação a 
objetos pode ser convertida para um equivalente com orientação a dados.

\section{Biblioteca Matemática}

Utilizando os conceitos discutidos na seção~\ref{secmathconcepts}, a intenção é ter uma 
biblioteca matemática o mais minimalista o possível, contendo somente as estruturas e 
funções necessárias para se realizar os testes e obter os resultados desejados deste 
trabalho. 
...
malhas e outros dados que são enviados aos shaders programáveis são armazenados em 
...
direta destes. Além disso, possui as seguintes funcionalidades:
\begin{itemize}
...
de pontos flutuantes. Esse limitante é utilizado para calcular a igualdade entre pontos 
flutuantes e consequentemente, todas as estruturas implementadas. A igualdade entre 
pontos flutuantes deve ser feita de maneira diferente da igualdade entre inteiros, isso 
...
O componente gráfico de um motor de jogos é um dos maiores módulos, devido a 
...
cena. As atualizações dessa parte visual consiste em alterações nas propriedades dos 
...
Para o motor deste trabalho, há apenas a implementação do \textit{vertex shader} e do 
\textit{fragment shader}, pois estes são os únicos que são indispensáveis para a 
construção do \textit{pipeline} de renderização, o restante dos \textit{shaders} 
possuem implementações padrões já fornecidas pelo OpenGL~\cite{shreiner2013opengl}.
...
\textit{shaders} os quais se deseja modificar ao longo da execução da aplicação. Suas 
...
uniforme se estende para todos os \textit{shaders} do programa de \textit{shader}, e 
não somente naquele que a variável uniforme foi declarada~\cite{wolff2013opengl}.

\subsection{Câmera}

A câmera dita o modo como a cena é visualizada pelo espectador. É basicamente 
...
cena e irá também cortar da renderização os objetos que estão fora do campo de visão 
da câmera. 

Uma câmera pode ter comportamentos diferentes, e este dependerá de como é pretendida a 
interação do espectador com a cena. Alguns exemplos de comportamento incluem:
\begin{itemize}
...
Podem existir mais de uma câmera na mesma cena e suas visões podem aparecer 
simultaneamente na tela, geralmente é feito um esquema de divisão de tela para que isso 
seja possível.

\subsection{Grafo de Cena}

O grafo de cena é o esquema de divisão espacial utilizado neste trabalho. Consiste em 
uma estrutura para definir a hierarquia dos objetos de uma cena e as propriedades de 
cada um.

Essa estrutura é um grafo acíclico direcionado cujos nós formam uma hierarquia. Cada 
...
O esquema de nós permite fácil reutilização de componentes, se dois objetos idênticos 
forem necessários na cena, basta criar um nó com as propriedades e utilizar 
instanciação, e também permite flexibilidade, cada nó pode ter sua própria malha, 
programa de \textit{shader}, entre outros atributos~\cite{hughes2014computer}. As 
ligações permitem uma fácil construção de hierarquia, e também torna trivial coisas 
como uma mudança de posição de um subgrupo inteiro de objetos.

\begin{figure}[h]
    \centering
    \captionof{figure}{Exemplo de um grafo de cena parcialmente descrevendo um objeto 
...
parcialmente representando um carro utilizando hierarquia, nele há um nó que 
...
frustum de visualização da câmera. Uma cena pode ser potencialmente extensa e conter 
objetos com formas complexas, por isso é importante que os parâmetros que definem as 
dimensões do frustum de visualização sejam adequadamente ajustados, para que objetos 
muito distantes ou fora do ângulo de visualização da câmera não sejam renderizados.
...
da memória, a qual é escarça. Por exemplo, vários objetos presentes em uma cena podem 
...
dados, algumas das estratégias escolhidas para a sua implementação e o que foi 
...
a dados como uma forma de otimização de desempenho, e também para a comparação com os 
jeitos mais tradicionais de se implementar um jogo.
...
utilizadas foram listadas. Além disso, algumas funcionalidades e convenções não 
...
Foi apresentado o conceito de um câmera, e possíveis comportamentos diferentes que 
