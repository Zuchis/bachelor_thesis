...
esperados através da determinação do fluxo de dados, utiliza o \textit{input} 
...
características de linguagens funcionais como Haskell ou ML adotadas pela linguagem 
...
conceitos: abstrações sem custo e posse. Esses dois conceitos serão explicados 
a seguir.
...
Tipos afins representam valores que são de propriedade exclusiva. Se uma variável que possui 
uma coleção for passada por valor a uma função, ou se sua coleção for atribuída a outra 
variável, a variável original perde a posse da coleção e o seu acesso. Para exemplificar 
o funcionamento dessa característica, a figura~\ref{rustmovedcollection} demonstra um 
dos possíveis erros gerados pelo \textit{borrow-checker}, componente do Rust responsável 
por detectar erros relacionados ao sistema de posse~\cite{rustdocs}. Nesse problema, o 
vetor $v$ tem uma certa coleção inicialmente atribuída a este. Um outro vetor $v2$ é 
declarado recebendo a coleção originalmente atribuída a $v$, este então perde a posse 
de sua coleção e não pode mais acessá-la, por isso as linhas 10 e 11 caracterizam um erro 
na linguagem, porque $v$ é uma variável desalocada e na linha 10 um método está sendo 
chamado para ela, enquanto que na linha 11 a variável está sendo indexada.

\begin{figure}[h]
    \centering
    \captionof{figure}{O problema de acesso a uma variável cuja coleção foi movida a 
    outra.}
    \includegraphics[width =.8\textwidth]{../figuras/rust-moved-value-error}
    \par\medskip
    Fonte: Autoria própria.
    \label{rustmovedcollection}
\end{figure}

Já na figura~\ref{rustcopyerror} há dois casos: em~\ref{fig:copyerror} ocorre um outro 
erro da linguagem, quando uma coleção é passada por valor a uma função e esta coleção não 
é retornada ao final da função. A variável que foi passada por valor também perde a posse 
e o acesso à sua coleção e por isso a linha 16 caracteriza um erro, quando se tenta 
atribuir a variável $v$ desalocada à $v2$. A figura~\ref{fig:copyfixed} mostra como esse 
problema pode ser resolvido, declarando na assinatura da função \textit{print\_vector} a variável 
$v1$ como uma referência a uma coleção, desta maneira, a variável original será passada como 
referência à função e não perderá a posse da sua coleção.

\begin{figure}[h!]
    \centering
    \captionof{figure}{O problema de conteúdo movido e uma possível maneira de 
    corrigi-lo.}
    \begin{subfigure}[b]{0.40\textwidth}
        \includegraphics[width=\textwidth]{../figuras/rust-copy-trait-error}
        \caption{}
        \label{fig:copyerror}
    \end{subfigure}
    \begin{subfigure}[b]{0.40\textwidth}
        \includegraphics[width=\textwidth]{../figuras/rust-copy-trait-error-fixed}
        \caption{}
        \label{fig:copyfixed}
    \end{subfigure}
    \par\medskip
...
definem uma região na qual os ponteiros que estão nela são válidos somente nesta região e 
