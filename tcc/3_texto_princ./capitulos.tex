...
esses \textit{arrays} das propriedades para garantir que permancem contíguos conforme 
...
esperados através da determinação do fluxo de dados, utiliza o input 
...
permitindo o uso eficiente de memória através processamento contíguo de dados. 
...
funcionalidades de linguagens funcionais como Haskell ou ML adotadas pela linguagem 
Rust incluem: tipos de dados algébricos, casamento de padrões e polimorfismo restringido. 
A diferença é que Rust é capaz de oferecer essas funcionalidades com um completo 
controle do \textit{layout} da memória e de sua alocação, assim como C e 
C++~\cite{holk2013}.
...
C++, mesmo sendo rápidas, não são seguras em termos de uso da memória. Apesar de ter 
sido estabilizada apenas há pouco tempo, seus objetivos permitem o desenvolvimento de 
programas robustos e a linguagem tem apresentado excelente desempenho. Seu código tem 
segurança de memória por padrão (a linguagem não permite duas referências mutáveis para 
uma mesma variável, elmininando assim, as condições de corrida), mais rápido que C++, 
mais fácil de manter e excelente em concorrência~\cite{Beingessner2016}.
...
Rust possui uma alta capacidade de otimização na compilação para permitir a construção de 
...
permitindo a aplicação de otimizações, como por exemplo a aplicação de 
\textit{inlining}~\cite{holk2013}.
...
lógica do jogo, ao oposto do que era feito na programação tradicional de jogos, cujo 
...
processado deste. Neste sistema as coordenadas são locais e o ponto de referência é o 
centro do objeto. Quando se define uma malha de polígonos, os vértices estão nesse 
...
anterior, este estágio é feito automaticamente pelo hardware. Após esse estágio, os 
...
Considerando os módulos citados, o \textit{loop} do jogo pode ser dividido da seguinte maneira, 
na seguinte ordem:
...
não precisa ser atualizada com a mesma frequência do que as simulações físicas, 
...
desenvolvimento do motor, será necessário o desenvolvimento de uma aplicação 
que utilize as funcionalidades deste para realmente verificar sua eficiência.

Para que seja possível um meio de comparação, será desenvolvida uma aplicação simples em 
duas versões diferentes, uma delas será feita com programação orientada a objetos, e na 
outra será utilizada modelagem orientada a dados. Isso permitirá a comparação entre 
essas duas abordagens e a análise de eficiência da MOD, bem como a demonstração de como 
uma aplicação com orientação a objetos pode ser convertida para um equivalente com 
orientação a dados.

\section{Biblioteca Matemática}

Utilizando os conceitos discutidos na seção~\ref{secmathconcepts}, a intenção é 
desenvolver uma biblioteca matemática o mais minimalista o possível, contendo somente 
...
Para o motor a ser desenvolvido neste trabalho, há apenas a implementação do 
