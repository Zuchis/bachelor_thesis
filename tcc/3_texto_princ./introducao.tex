...
em torno do problema e suas possíveis soluções, e não em torno dos dados. Ou seja, fornece abstração dos dados porém a 
custo do desempenho.
...
mudança da perspectiva da programação dos objetos para os dados, seus tipos, como estão armazenados na memória e como serão 
lidos e processados~\cite{fabiandod}. Para alcançar esses objetivos, essa técnica sugere dividir cada objeto em diferentes 
...
Todos esses elementos, técnicas e otimizações do motor gráfico no fim constroem o que é conhecido como \textit{pipeline} de 
renderização. A principal função desse \textit{pipeline} é renderizar uma imagem bidimensional, 
